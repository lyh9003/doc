...
if 'user_name' in st.session_state and 'user_number' in st.session_state:
    ...
    이때 반드시 아래의 [문제] 전체를 보여줘서 학생이 글의 내용을 모두 읽고 파악할 수 있도록 해주세요. 이 때 [정답] 전체를 노출하지 마세요.
    ...
    "안녕하세요, 저는 B선생님입니다. 아래 지문은 우리나라 광복절에 대한 이야기입니다. 각 문장별로 잘못된 부분이 1개 있습니다.
    먼저 전체 지문을 읽으며 맥락을 파악해 볼까요?"
